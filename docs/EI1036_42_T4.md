...
-  *Atajos* (shortcode): permiten añadir funcionalidad de algunos plugins en nuestra página web. Es  un  texto entre las marcas [ ], que  cuando la página es presentada al visitante, lo que hay entre las marcas se sustituye por el resultado de la función concreta que se ha invocado.[woocommerce_cart]
...
#  Custom Fields para las entradas
...
Un callback es un tipo de funciones que son pasadas como parámetros y que serán ejecutadas desde una subrutina. 
...
*  Actividad: Comentar el pages.php de un tema.
...
* Problema: Mostrar el pie de página en la página principal, pero no en ninguna otra página

Solución Problema2: agregue  ```get_footer() ``` al final de index.php, pero no en page.php.
...
- is_home()
   ...
   está en la página principal configurada
- is_front_page()

  está en la pàgina front_page.php
- is_single('excursion-a-piedramillera')

  está la entrada denominada “Excursión a Piedramillera”;

- in_category('sesion-1')

  está  la entrada de la categoría “Sesión 1”. 
...
Los hooks de accion o acciones son disparadas cuando  pasa algo, como que se cargue la página, se  inicie una sesión...
...
* tiene tres parámetros : el nombre del hook de filtro, el valor que quieres filtrar(ejem. el valor por default), y variables opcionales las cuales se pasan  al callback del filtro.
...
### Cuestión: ¿Cómo genera la página principal html en WP?
