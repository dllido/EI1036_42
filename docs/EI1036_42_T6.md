...
Recuerda  que pera carrgar un script en wordpress la mejor forma es:

En functions.php cargar   wp_register_script('miscript',get_stylesheet_directory_uri().'/js/script.js' );

Y en la página amb el codi HTML que volem que se execute:

wp_enqueue_script('miscript');

Si hay un código que queremos que se ejecute siempre, y que se cargue en el footer podemos evitar el registro y simplemente llamar al hook de wordpress:
